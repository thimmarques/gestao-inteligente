...
import {
    ...
    Circle, Clock, AlertTriangle, Trash2, Edit2, ChevronRight, ListTodo
} from 'lucide-react';
...
const Tasks: React.FC = () => {
    ...
    const handleToggleStatus = async (task: Task) => {
        ...
        await taskService.updateTask(task.id, {
            status: newStatus,
            completed_at: newStatus === 'concluído' ? new Date().toISOString() : undefined
        });
        refetch();
    };
        ...
        <div className="p-6 md:p-10 space-y-8 min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white pb-24 animate-in fade-in duration-500">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                    ...
                    <h1 className="text-3xl font-black tracking-tight">Gerenciamento de Tarefas</h1>
                </div>
                <button
            ...
            {isAdding && (
                <form onSubmit={handleAddTask} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border-2 border-primary-500 shadow-xl animate-in slide-in-from-top-4 duration-300">
                    <input
                        autoFocus
                        type="text"
                    ...
                    <div className="flex justify-end gap-3">
                        <button type="button" onClick={() => setIsAdding(false)} className="px-4 py-2 text-slate-500 font-bold">Cancelar</button>
                        <button type="submit" className="px-6 py-2 bg-primary-600 text-white rounded-xl font-bold shadow-lg shadow-primary-500/20">Criar Tarefa</button>
                    </div>
                </form>
            )}
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Carregando tarefas...</p>
                </div>
                    ...
                    {filteredTasks.map(task => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <h3 className={`font-bold transition-all ${task.status === 'concluído' ? 'line-through text-slate-500' : 'text-slate-800 dark:text-white'}`}>
                                    {task.title}
                                </h3>
                                {task.due_date && (
                                    <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-1">
                                        <Clock size={12} />
                                        <span>Vence em: {new Date(task.due_date).toLocaleDateString()}</span>
                                    </div>
                                )}
                            </div>
                ...
                <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-[3rem] border border-dashed border-slate-300 dark:border-slate-800">
                    ...
                    <p className="text-slate-500 font-medium font-serif italic text-lg">Parece que você está em dia com tudo! 🎉</p>
                </div>
            )}
        </div>
    );
