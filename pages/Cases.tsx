...
import {
  Folder, Plus, Search, Filter, Loader2, MoreVertical,
  ExternalLink, Clock, User, Hash, Tag, Scale, Edit2
} from 'lucide-react';
...
const getStatusColor = (status: string) => {
  switch (status) {
    case 'distribuído': return 'bg-blue-100 text-blue-600 dark:bg-blue-900/30';
    case 'andamento': return 'bg-green-100 text-green-600 dark:bg-green-900/30';
    case 'sentenciado': return 'bg-purple-100 text-purple-600 dark:bg-purple-900/30';
    case 'arquivado': return 'bg-slate-100 text-slate-600 dark:bg-slate-800';
...
const Cases: React.FC = () => {
  ...
  const filteredCases = useMemo(() => {
    return cases.filter(c => {
      const matchesSearch = c.process_number.includes(searchTerm) ||
    ...
    <div className="p-6 md:p-10 space-y-8 min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white pb-24 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Gerenciamento de {cases.length} processos judiciais.</p>
        </div>
        <button
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto">
          {['todos', 'distribuído', 'andamento', 'sentenciado', 'recurso'].map(s => (
            <button
              key={s}
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Carregando processos...</p>
        </div>
      ) : filteredCases.length > 0 ? (
        <div className="grid grid-cols-1 gap-4">
          {filteredCases.map(c => (
            <div key={c.id} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:translate-x-1 transition-all group relative overflow-hidden">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex-1 space-y-4">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${getStatusColor(c.status)}`}>
                      {c.status}
                    </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                        <User size={14} className="text-primary-500" /> {c.client?.name || 'Não vinculado'}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Tribunal / Vara</p>
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-100">{c.court}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Valor da Causa</p>
                      <p className="text-sm font-black text-primary-600">{formatCurrency(c.value)}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Iniciado em</p>
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                        <Clock size={14} className="text-slate-400" /> {new Date(c.started_at).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  ...
                  {c.tags && c.tags.length > 0 && (
                    <div className="flex gap-2 pt-2">
                      {c.tags.map(t => (
                        <span key={t} className="px-2 py-0.5 bg-slate-50 dark:bg-slate-800 text-[10px] font-bold text-slate-400 rounded-full border border-slate-100 dark:border-slate-700">
                          #{t}
                        </span>
                      ))}
                    </div>
        ...
        <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-[3rem] border border-dashed border-slate-300 dark:border-slate-800">
          ...
          <p className="text-slate-500 font-medium font-serif italic text-lg">Nenhum processo em andamento.</p>
        </div>
      )}
