...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    case 'arquivado':
      return 'bg-slate-100 text-slate-600 dark:bg-navy-800';
    default:
          ...
          {['todos', 'distribuído', 'andamento', 'sentenciado', 'recurso'].map(
            (s) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${statusFilter === s ? 'bg-primary-600 text-white shadow-md' : 'bg-slate-50 dark:bg-navy-800 text-slate-500'}`}
              >
                {s}
              </button>
            )
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredCases.map((c) => (
            <div
              key={c.id}
              className="bg-white dark:bg-navy-800/50 p-6 rounded-3xl border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-xl hover:translate-x-1 transition-all group relative overflow-hidden"
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex-1 space-y-4">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-navy-800 text-slate-500 rounded-lg text-xs font-bold font-mono">
                    ...
                    <span className="flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-navy-800 text-slate-500 rounded-lg text-xs font-bold capitalize">
                  ...
                  {c.tags && c.tags.length > 0 && (
                    <div className="flex gap-2 pt-2">
                      {c.tags.map((t) => (
                        <span
                          key={t}
                          className="px-2 py-0.5 bg-slate-50 dark:bg-navy-800 text-[10px] font-bold text-slate-400 rounded-full border border-slate-100 dark:border-white/15"
                        >
                          #{t}
                        </span>
                      ))}
                ...
                <div className="flex md:flex-col gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 md:flex-none p-3 bg-slate-50 dark:bg-navy-800 text-slate-400 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
                    title="Ver Detalhes"
                  >
