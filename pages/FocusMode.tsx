...
import { Play, Pause, RotateCcw, CheckCircle2, Circle, ArrowLeft, Target, Settings, Plus } from 'lucide-react';
...
const FocusMode: React.FC = () => {
  ...
  const [tasks, setTasks] = useState<{id: string, title: string, completed: boolean, priority: string}[]>([]);
  ...
  useEffect(() => {
    ...
    if (isActive && timeLeft > 0) {
      ...
      alert(isBreak ? "Pausa terminada! De volta ao trabalho." : "Hora de uma pausa!");
    }
  ...
  const handleAddTask = () => {
    ...
    if (title) {
      setTasks([...tasks, { id: Date.now().toString(), title, completed: false, priority: 'média' }]);
    }
  };
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="relative flex items-center justify-center mb-12">
            <svg className="w-80 h-80 -rotate-90">
              <circle cx="160" cy="160" r="150" className="stroke-slate-800" strokeWidth="10" fill="transparent" />
              <circle 
                cx="160" 
                cy="160" 
                r="150" 
                ...
                fill="transparent" 
                strokeDasharray={942} 
                strokeDashoffset={942 - (942 * (timeLeft / (isBreak ? 5 * 60 : 25 * 60)))}
                strokeLinecap="round"
              />
            </svg>
        ...
        <div className="bg-slate-800/50 p-8 rounded-3xl border border-slate-700 shadow-2xl backdrop-blur-sm h-fit min-h-[400px]">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-sm font-medium text-slate-400">
              {tasks.filter(t => t.completed).length}/{tasks.length} concluídos
            </span>
          </div>
          ...
          <div className="space-y-4">
            {tasks.length > 0 ? (
              tasks.map((task) => (
                ...
                >
                  {task.completed ? (
                    ...
                    <Circle className="text-slate-600 group-hover:text-primary-400" size={24} />
                  )}
              ...
              <div className="py-20 text-center text-slate-500 border border-dashed border-slate-700 rounded-2xl">
                 <p className="text-sm italic">Adicione tarefas para manter o foco.</p>
              </div>
            )}
          </div>
