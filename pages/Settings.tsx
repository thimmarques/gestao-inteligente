    ...
    <div className="p-4 md:p-6 space-y-6 min-h-screen bg-slate-50 dark:bg-slate-950 animate-in fade-in duration-500 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
            ...
            <span className="text-primary-600">Configurações</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-black dark:text-white tracking-tight mt-1 flex items-center gap-3">
            Ajustes do Sistema
            <SettingsIcon
              className="text-slate-300 animate-spin-slow hidden sm:block"
              size={24}
            />
          </h1>
          <p className="text-slate-500 text-sm font-medium max-w-xl mt-1">
            Gerencie sua identidade visual, segurança e rastreabilidade.
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
        <aside className="space-y-3">
          <div
            className={`flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 scrollbar-hide -mx-4 px-4 lg:mx-0 lg:px-0`}
          >
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col items-start leading-none gap-0.5">
                    <span
                      className={`text-[10px] font-bold uppercase tracking-wide ${isActive ? 'text-slate-900 dark:text-white' : 'text-slate-500'}`}
                    >
                      {item.label}
                    </span>
                    {item.alert && (
                  ...
                  {isActive && (
                    <ChevronRight
                      size={14}
                      className="ml-auto text-primary-600 hidden lg:block"
                    />
                  )}
                </button>
              );
