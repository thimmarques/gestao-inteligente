...
const setupHeader = (doc: jsPDF, office: Office, lawyer: Lawyer | null) => {
  ...
  doc.text(officeAddr.toUpperCase(), PAGE_WIDTH - MARGIN_RIGHT, 14, {
    align: "right",
  });
  doc.setFont("helvetica", "normal");
  doc.setTextColor(200, 200, 200);
  doc.text(`OAB/${lawyerOab} | ${lawyerEmail}`, PAGE_WIDTH - MARGIN_RIGHT, 19, {
    align: "right",
  });
...
const drawSectionHeader = (doc: jsPDF, title: string, y: number) => {
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(
    COLOR_SECTION_TEXT[0],
    COLOR_SECTION_TEXT[1],
    COLOR_SECTION_TEXT[2],
  );
...
const setupFooter = (doc: jsPDF, clientName: string, city: string) => {
  ...
  doc.text(clientName.toUpperCase(), PAGE_WIDTH / 2, lineY + 8, {
    align: "center",
  });
};
...
export const generateProcuracaoPDF = async ({
  client,
  lawyer,
  office,
}: DocumentParams) => {
  ...
  const lawyerName = (
    lawyer?.name || "THIAGO MASTRANGELO MARQUES"
  ).toUpperCase();
  const lawyerOab = lawyer?.oab || "410.047/SP";
  const officeAddr = office.address || "R: Manoel Sichieri, 1089";
  ...
  const finalidadeText =
    ...
    "A presente procuração destina-se à representação plena do outorgante perante órgãos judiciais e administrativos, visando a defesa de seus direitos e interesses no processo em epígrafe.";
  currentY = addJustifiedText(
    doc,
    finalidadeText,
    MARGIN_LEFT,
...
export const generateDeclaracaoHipossuficienciaPDF = async ({
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(COLOR_HEADER_BG[0], COLOR_HEADER_BG[1], COLOR_HEADER_BG[2]);
  doc.text("DECLARAÇÃO DE HIPOSSUFICIÊNCIA", PAGE_WIDTH / 2, 50, {
    align: "center",
  });
  ...
  const income = client.income
    ? `R$ ${parseFloat(client.income).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`
    : "R$ 0,00";
