...
export async function generateProductivityReportPDF(
  ...
  const periodCases = cases.filter((c) =>
    isWithinInterval(new Date(c.started_at), {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        "Prazos Concluídos",
        periodDeadlines.filter((d) => d.status === "concluído").length,
      ],
      [
        "Taxa de Compliance",
        `${((periodDeadlines.filter((d) => d.status === "concluído").length / (periodDeadlines.length || 1)) * 100).toFixed(1)}%`,
      ],
    ],
    theme: "grid",
