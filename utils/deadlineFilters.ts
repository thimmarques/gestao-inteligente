...
export function sortDeadlines(
  ...
  sorted.sort((a, b) => {
    ...
    if (sort.field === 'deadline_date') {
      comparison = new Date(a.deadline_date).getTime() - new Date(b.deadline_date).getTime();
      ...
      comparison = (a.case?.process_number || '').localeCompare(b.case?.process_number || '');
    } else if (sort.field === 'client') {
      comparison = (a.case?.client.name || '').localeCompare(b.case?.client.name || '');
    } else if (sort.field === 'status') {
      const statusOrder: Record<string, number> = { 'vencido': 0, 'pendente': 1, 'concluído': 2 };
