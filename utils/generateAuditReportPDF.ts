...
export async function generateAuditReportPDF(
  ...
  doc.text('RELATÓRIO DE AUDITORIA E SEGURANÇA', pageWidth - 15, 24, { align: 'right' });
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  const sectionText = config.filters.section === 'all' ? 'Sistema Completo' : `Seção: ${config.filters.section.toUpperCase()}`;
  doc.text([
    `Escopo: ${sectionText}`,
  ...
  const detailedData = logs.map(l => [
    ...
    l.entity_description.length > 50 ? l.entity_description.substring(0, 50) + '...' : l.entity_description,
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(`AUDIT_HASH: ${Math.random().toString(36).substring(7).toUpperCase()}`, 15, pageHeight - 10);
  }
