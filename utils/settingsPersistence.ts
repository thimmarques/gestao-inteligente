...
export function updateLawyer(updates: Partial<Lawyer>): void {
  ...
  if (session) {
    ...
    localStorage.setItem(STORAGE_KEYS.SESSION, JSON.stringify({ ...parsed, ...updates }));
  }
  ...
  if (teamRaw) {
    ...
    const index = team.findIndex((m: any) => m.id === current.id || m.email === current.email);
    if (index !== -1) {
      team[index] = { ...team[index], ...updates, updated_at: new Date().toISOString() };
...
export function getOffice(): Office {
  ...
  return data ? JSON.parse(data) : { id: 'office-1', name: 'LegalTech' } as Office;
}
