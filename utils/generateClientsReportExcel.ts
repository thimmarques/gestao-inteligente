...
export async function generateClientsReportExcel(
  ...
  const summaryData = [
    ...
    ['Clientes Particulares', clients.filter(c => c.type === 'particular').length],
    ['Clientes Defensoria', clients.filter(c => c.type === 'defensoria').length],
  ...
  const clientsData = clients.map(c => {
    const clientRevenues = finances.filter(f => 
      f.client_id === c.id && 
      f.type === 'receita' && 
      f.status === 'pago'
    );
