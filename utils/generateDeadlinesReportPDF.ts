...
export async function generateDeadlinesReportPDF(
  config: ReportConfig,
  deadlines: Deadline[]
): Promise<string> {
  const doc = new jsPDF();
  ...
  const periodDeadlines = deadlines.filter(d =>
    isWithinInterval(new Date(d.deadline_date), {
  ...
  doc.setFontSize(22);
  doc.setFont('helvetica', 'bold');
  doc.text('Relatório de Prazos Judiciais', 15, 60);

  autoTable(doc, {
    startY: 75,
    head: [['Vencimento', 'Ato Processual', 'Processo', 'Prioridade', 'Status']],
    body: periodDeadlines.map(d => [
      format(new Date(d.deadline_date), 'dd/MM/yyyy'),
      d.title,
      d.case?.process_number || '---',
  ...
  applyProfessionalLayout(doc, 'Controladoria Jurídica');

  const fileName = `prazos_${format(new Date(), 'yyyyMMdd')}.pdf`;
  doc.save(fileName);
  return fileName;
}
