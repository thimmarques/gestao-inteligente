...
import { addMonths, format, subMonths, isWithinInterval, startOfMonth, endOfMonth } from 'date-fns';
...
export function calculateForecast(
  ...
  const last6MonthsRevenues = revenues.filter(r =>
    r.type === 'receita' &&
    r.status === 'pago' &&
    r.paid_date &&
    r.category !== 'Retainer Fee (Mensalidade)' &&
    isWithinInterval(new Date(r.paid_date), { start: sixMonthsAgo, end: now })
  );
  const variableRevenueAvg = last6MonthsRevenues.reduce((sum, r) => sum + r.amount, 0) / 6;

  // 3. Despesas Fixas (Padrão: aluguel, software, marketing do último mês)
  const lastMonthStart = startOfMonth(subMonths(now, 1));
  const lastMonthEnd = endOfMonth(subMonths(now, 1));
  const fixedExpenseCategories = ['Despesas Operacionais', 'Software e Tecnologia', 'Marketing'];
  
  const lastMonthFixedExpenses = expenses.filter(e =>
    e.type === 'despesa' &&
    fixedExpenseCategories.includes(e.category) &&
    e.status === 'pago' &&
    e.paid_date &&
    isWithinInterval(new Date(e.paid_date), { start: lastMonthStart, end: lastMonthEnd })
  ...
  const last6MonthsExpenses = expenses.filter(e =>
    ...
    e.paid_date &&
    !fixedExpenseCategories.includes(e.category) &&
    isWithinInterval(new Date(e.paid_date), { start: sixMonthsAgo, end: now })
  );
  const variableExpenseAvg = last6MonthsExpenses.reduce((sum, e) => sum + e.amount, 0) / 6;
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    const projected_expenses = (lastMonthFixedExpenses || 2500) + variableExpenseAvg; // Fallback para fixo se vazio
