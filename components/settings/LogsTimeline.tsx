...
import { Eye, User, Plus, Edit, Trash2, Shield, LogIn, LogOut, ShieldAlert, Download, Database } from 'lucide-react';
...
export const LogsTimeline: React.FC<LogsTimelineProps> = ({ logs, onExpand }) => {
              ...
              {dayLogs.map((log: AuditLog) => {
                ...
                return (
                  <div key={log.id} className="relative pl-12 group animate-in slide-in-from-left-4 duration-300">
                    <div className={`absolute left-[11px] top-1.5 w-2.5 h-2.5 rounded-full z-10 border-2 border-white dark:border-slate-950 ${color}`} />
                    ...
                    >
                      <header className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-bold dark:text-white uppercase tracking-tighter">{log.lawyer_name}</span>
                        </div>
                        <span className="text-[10px] font-medium text-slate-400">
                          {format(new Date(log.timestamp), 'HH:mm:ss')} • {formatDistanceToNow(new Date(log.timestamp), { addSuffix: true, locale: ptBR })}
                        </span>
                      </header>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm text-slate-600 dark:text-slate-300">
                          <span className="font-bold uppercase text-[10px] mr-1">{log.action.replace('_', ' ')}</span>
                      ...
                      <div className="mt-4 flex items-center justify-between">
                         <div className="flex gap-2">
                           <span className="px-2 py-0.5 rounded-md bg-slate-50 dark:bg-slate-800 text-[8px] font-bold text-slate-400 uppercase border border-slate-100 dark:border-slate-700">{log.entity_type}</span>
                           <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase ${
                             log.criticality === 'crítico' ? 'text-red-500 bg-red-50' : 'text-slate-500 bg-slate-50'
                           }`}>{log.criticality}</span>
                         </div>
                         <button className="text-[10px] font-black text-primary-600 uppercase tracking-widest opacity-0 group-hover/card:opacity-100 transition-opacity">Detalhes</button>
                      </div>
                    </div>
                  </div>
                );
