...
import { Lock, Eye, EyeOff, ShieldAlert, LogOut, Check, X, ShieldCheck, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-10">
      <section className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-10">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 rounded-xl"><Lock size={20} /></div>
          <h3 className="text-sm font-black dark:text-white uppercase tracking-widest">Segurança (Campos Opcionais)</h3>
        </div>

        <form onSubmit={handlePasswordSubmit} className="max-w-md space-y-8">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Senha Atual</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Nova Senha</label>
            <div className="relative">
              <input 
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Confirmar Nova Senha</label>
            <input 
              type="password"
          ...
          >
            {isChanging ? <Loader2 size={18} className="animate-spin" /> : <Check size={18} />}
            Alterar Senha
          </button>
        </form>
      </section>
      ...
      <section className="p-10 rounded-[3rem] border-2 border-red-500/20 bg-red-500/5 space-y-10">
        ...
        <button onClick={handleDeleteAccount} className="px-10 py-4 border-2 border-red-600 text-red-600 hover:bg-red-600 hover:text-white rounded-[1.5rem] font-black uppercase tracking-[0.2em] transition-all">Excluir Conta</button>
      </section>
    </div>
  );
};
