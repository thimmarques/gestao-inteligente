...
import { Globe, RefreshCw, LogOut, CheckCircle2, AlertCircle, Loader2, FileText } from 'lucide-react';
        ...
        <div className="p-8 md:p-10 flex flex-col md:flex-row gap-10">
          ...
          <div className="flex-1 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-xl font-bold dark:text-white">Google Calendar</h3>
                <p className="text-sm text-slate-500 mt-1">Sincronize audiências, reuniões e prazos fatais com sua agenda pessoal.</p>
              </div>
              <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest self-start md:self-center flex items-center gap-2 border ${
                isConnected ? 'bg-green-50 text-green-600 border-green-200' : 'bg-slate-100 text-slate-500 border-slate-200'
              }`}>
              ...
              <div className="space-y-6">
                <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[2rem] grid grid-cols-1 sm:grid-cols-2 gap-6 border border-slate-100 dark:border-slate-800">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Conta Conectada</p>
                  ...
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Última Sincronização</p>
                    <p className="text-sm font-bold dark:text-white flex items-center gap-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={handleDisconnect} className="flex items-center gap-2 px-6 py-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl text-xs font-bold transition-all">
                    <LogOut size={16} />
                    Desconectar Agenda
                  </button>
                </div>
              ...
              <div className="space-y-8">
                <div className="p-6 bg-blue-50 dark:bg-blue-900/10 rounded-[2rem] border border-blue-100 dark:border-blue-900/30 flex gap-4">
                  ...
                  <p className="text-sm text-primary-900 dark:text-primary-300 font-medium leading-relaxed">
                    Ao conectar, o LegalTech criará automaticamente eventos para todas as suas audiências agendadas e enviará lembretes baseados em sua agenda pessoal.
                  </p>
                </div>
                <button
                ...
                >
                  {isConnecting ? <Loader2 size={24} className="animate-spin" /> : <Globe size={24} />}
