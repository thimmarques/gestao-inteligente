  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <IndicatorCard 
        ...
        color={suspiciousActivity.denied > 5 ? 'text-red-500' : 'text-green-500'}
        items={[
          { label: 'Acessos negados', value: suspiciousActivity.denied, ok: suspiciousActivity.denied < 3 },
...
const IndicatorCard = ({ title, icon, score, color, items }: any) => (
  <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:shadow-md">
    <div className="flex items-center justify-between mb-5">
      ...
      <span className={`text-base font-black ${color} tracking-tight`}>{score}</span>
    </div>
    <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-4">{title}</h4>
    <div className="space-y-2.5">
      {items.map((item: any, i: number) => (
        <div key={i} className="flex items-center justify-between">
          <span className="text-[11px] font-medium text-slate-500 truncate mr-3">{item.label}</span>
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-[11px] font-black dark:text-white tabular-nums">{item.value}</span>
            {item.ok ? (
