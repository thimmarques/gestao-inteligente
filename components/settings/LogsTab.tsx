...
import {
  FileSearch, Download, Activity, AlertTriangle,
  Search, ChevronDown, RefreshCw, ShieldCheck,
  History, List, Layers, Eye, Loader2
} from 'lucide-react';
...
import { checkSecurityAlerts, SecurityAlert } from '../../utils/securityAlerts.ts';
...
export const LogsTab: React.FC = () => {
  const { lawyer } = useApp();
  const { data: logs = [], isLoading, refetch } = useAuditLogs(lawyer?.office_id);
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter(l => {
      const searchableText = [
        l.lawyer_name, l.entity_description, l.action
      ].join(' ').toLowerCase();

      const matchSearch = searchableText.includes(searchTerm.toLowerCase());
      const matchSection = selectedSection === 'all' || l.entity_type === selectedSection;

      const logDate = new Date(l.created_at || l.timestamp);
      let matchPeriod = true;
      if (period === 'today') matchPeriod = isAfter(logDate, startOfDay(new Date()));
      else if (period === '7d') matchPeriod = isAfter(logDate, subDays(new Date(), 7));
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-lg font-black dark:text-white tracking-tight">Audit Log</h2>
            <div className="relative mt-1">
              ...
              >
                <Layers size={12} />
                Visualizando: <span className="text-primary-600">{sections.find(s => s.id === selectedSection)?.label}</span>
                <ChevronDown size={12} />
              </button>
              ...
              {isSectionMenuOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-50 p-2 animate-in zoom-in-95">
                  {sections.map(s => (
                    <button
                      key={s.id}
                      onClick={() => { setSelectedSection(s.id); setIsSectionMenuOpen(false); }}
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleExportContextual} className="flex items-center gap-2 px-6 py-2.5 bg-slate-950 text-white dark:bg-primary-600 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all">
            <Download size={14} /> Exportar
          </button>
          <button onClick={() => refetch()} className="p-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 hover:text-primary-600">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/30">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Registros Cronológicos</h3>
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
            <button onClick={() => setViewMode('table')} className={`p-1.5 rounded-md transition-all ${viewMode === 'table' ? 'bg-white dark:bg-slate-700 text-primary-600 shadow-sm' : 'text-slate-400'}`}><List size={16} /></button>
            <button onClick={() => setViewMode('timeline')} className={`p-1.5 rounded-md transition-all ${viewMode === 'timeline' ? 'bg-white dark:bg-slate-700 text-primary-600 shadow-sm' : 'text-slate-400'}`}><History size={16} /></button>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  <th className="px-8 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Data/Hora</th>
                  <th className="px-6 py-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Seção</th>
                  ...
                  <th className="px-8 py-4 text-right text-[9px] font-black text-slate-400 uppercase tracking-widest">Detalhes</th>
                </tr>
              </thead>
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={5} className="px-8 py-20 text-center">
                      ...
                      <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Carregando registros de auditoria...</p>
                    </td>
                  </tr>
                ) : filteredLogs.map(log => (
                  <React.Fragment key={log.id}>
                    <tr onClick={() => setExpandedLogId(expandedLogId === log.id ? null : log.id)} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 cursor-pointer group">
                      <td className="px-8 py-4"><span className="text-[11px] font-bold dark:text-slate-300 tabular-nums">{format(new Date(log.created_at || log.timestamp), 'dd/MM HH:mm')}</span></td>
                      <td className="px-6 py-4"><span className="text-[9px] font-black uppercase text-slate-400">{log.entity_type}</span></td>
                      <td className="px-6 py-4"><span className="text-[10px] font-black uppercase dark:text-white">{log.action}</span></td>
                      <td className="px-6 py-4"><span className="text-xs font-medium dark:text-slate-400 truncate max-w-xs block">{log.entity_description}</span></td>
                      <td className="px-8 py-4 text-right"><Eye size={16} className="ml-auto text-slate-300 group-hover:text-primary-50" /></td>
                    </tr>
                    {expandedLogId === log.id && (
                      <tr>
                        <td colSpan={5} className="p-6 bg-slate-50/50 dark:bg-slate-950/40 border-y border-slate-100 dark:border-slate-800">
