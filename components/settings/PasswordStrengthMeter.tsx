...
export const PasswordStrengthMeter: React.FC<PasswordStrengthMeterProps> = ({ password }) => {
  const strength = useMemo(() => {
    if (!password) return { score: 0, label: 'Vazia', color: 'bg-slate-200', textColor: 'text-slate-400' };
    ...
    const results = [
      ...
      { score: 2, label: 'Média', color: 'bg-yellow-500', textColor: 'text-yellow-500' },
      { score: 3, label: 'Forte', color: 'bg-green-500', textColor: 'text-green-500' },
      { score: 4, label: 'Muito Forte', color: 'bg-emerald-500', textColor: 'text-emerald-500' },
    ];
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
        {requirements.map((req, i) => (
          <div key={i} className={`flex items-center gap-2 text-[9px] font-bold uppercase tracking-tight transition-colors ${req.met ? 'text-green-600' : 'text-slate-400'}`}>
