...
export const DeadlinesTab: React.FC<DeadlinesTabProps> = ({ caseId }) => {
  ...
  const getStatusInfo = (date: string, status: string) => {
    ...
    if (diff < 0) return { color: 'border-slate-400', bg: 'bg-slate-400', text: `Vencido há ${Math.abs(diff)} dias`, labelColor: 'text-slate-500' };
    if (diff === 0) return { color: 'border-red-600', bg: 'bg-red-600', text: 'Vence HOJE', labelColor: 'text-red-600' };
    if (diff <= 3) return { color: 'border-red-500', bg: 'bg-red-500', text: `Faltam ${diff} dias`, labelColor: 'text-red-500' };
    if (diff <= 7) return { color: 'border-yellow-500', bg: 'bg-yellow-500', text: `Faltam ${diff} dias`, labelColor: 'text-yellow-600' };
    return { color: 'border-green-500', bg: 'bg-green-500', text: `Faltam ${diff} dias`, labelColor: 'text-green-600' };
  };
    ...
    <div className="space-y-6 relative animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h3 className="text-xl font-bold dark:text-white">Prazos Processuais</h3>
      ...
      <div className="grid grid-cols-1 gap-4">
        {deadlines.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-1">
                      <h4 className="font-bold text-sm dark:text-white">{deadline.title}</h4>
                      <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 text-center">
            ...
            <h4 className="text-lg font-bold dark:text-white mb-2">Nenhum prazo cadastrado</h4>
            <p className="text-slate-500 text-sm mb-6 max-w-xs">Mantenha seu escritório organizado adicionando prazos fatais.</p>
          </div>
        )}
      </div>
    </div>
