      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Total Receitas</span>
          </div>
          <p className="text-3xl font-bold text-green-600 tracking-tight">{formatCurrency(totalRevenue)}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Total Despesas</span>
          </div>
          <p className="text-3xl font-bold text-red-600 tracking-tight">{formatCurrency(totalExpense)}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Saldo Processo</span>
          </div>
          <p className={`text-3xl font-bold tracking-tight ${balance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                records.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${r.type === 'receita' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                        {r.type === 'receita' ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-medium dark:text-white">{r.category}</td>
                    <td className={`px-6 py-4 text-sm font-bold tabular-nums ${r.type === 'receita' ? 'text-green-600' : 'text-red-600'}`}>
