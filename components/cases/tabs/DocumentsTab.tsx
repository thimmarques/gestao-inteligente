...
import { Upload, FileText, Image as ImageIcon, Download, Trash2, X, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      >
        ...
        {isUploading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-bold text-primary-600">Enviando arquivos...</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              <h4 className="text-lg font-bold dark:text-white">Arraste arquivos ou clique para selecionar</h4>
              <p className="text-sm text-slate-500 mt-1">Formatos aceitos: PDF, DOCX, JPG, PNG • Máximo 10MB por arquivo</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-6">
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
          Documentos Anexados
          <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-md text-[10px] font-bold text-slate-500">{documents.length}</span>
        ...
        {documents.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {documents.map((doc) => (
              <div key={doc.id} className="group relative bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all">
                <div className="flex flex-col items-center text-center gap-3">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                    doc.type === 'pdf' ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-blue-500'
                  }`}>
                    {['jpg', 'png'].includes(doc.type) ? <ImageIcon size={24} /> : <FileText size={24} />}
                  </div>
                  <div className="w-full">
                    <p className="text-sm font-bold dark:text-white truncate px-2" title={doc.name}>{doc.name}</p>
          ...
          <div className="py-20 text-center bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800">
            <p className="text-slate-400 italic">Nenhum documento anexado ao processo ainda.</p>
          </div>
        )}
      </div>
    </div>
