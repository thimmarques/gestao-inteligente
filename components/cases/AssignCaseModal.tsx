...
import { X, UserPlus, Mail, MessageSquare, Loader2, Check, Shield } from 'lucide-react';
...
export const AssignCaseModal: React.FC<AssignCaseModalProps> = ({
  caseIds, currentLawyerId, isOpen, onClose, onAssign
}) => {
  const [lawyers, setLawyers] = useState<TeamMember[]>([]);
  const [selectedLawyerId, setSelectedLawyerId] = useState(currentLawyerId || '');
  ...
  useEffect(() => {
    ...
    setLawyers(team.filter((m: any) => m.role !== 'assistente' && m.status === 'ativo'));
  }, [isOpen]);
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col animate-in zoom-in-95 duration-300">
        <div className="px-8 pt-8 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold dark:text-white tracking-tight">Atribuir Processo</h2>
          ...
          <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">{caseIds.length} processo(s) selecionado(s)</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          {currentLawyer && (
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden shrink-0">
                {currentLawyer.photo_url ? <img src={currentLawyer.photo_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs font-bold text-slate-500">{currentLawyer.name[0]}</div>}
              </div>
              <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Atribuído atualmente a:</p>
                <p className="text-sm font-bold dark:text-white leading-tight">{currentLawyer.name}</p>
              </div>
            </div>
          )}

          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Atribuir para*</label>
            <select
              required
            ...
            >
              ...
              {lawyers.map(l => (
                <option key={l.id} value={l.id}>{l.name} {l.oab ? `(OAB/${l.oab})` : ''}</option>
              ))}
            </select>
          </div>
          ...
          <div className="space-y-4 pt-2">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold dark:text-slate-300">Notificar Advogado</span>
                </div>
                <button 
                  type="button"
                ...
                >
                  <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-all ${notify ? 'translate-x-6' : ''}`} />
                </button>
             </div>
          ...
          <div className="pt-4 flex flex-col gap-3">
            ...
            <button type="button" onClick={onClose} className="w-full py-3 text-xs font-bold text-slate-400 hover:text-slate-600 uppercase tracking-widest transition-all">Cancelar</button>
          </div>
        </form>
      </div>
    </div>
