  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${statusColorMap[caseData.status.toLowerCase()] || 'bg-slate-100'}`}>
          {caseData.status}
        </span>
      </div>
      ...
      <div className="space-y-4 mb-6 flex-1">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold dark:text-white truncate">Cliente {caseData.client_id}</p>
        ...
        <div className="flex items-center gap-2">
          <Scale size={14} className={typeColorMap[caseData.type.toLowerCase()] || 'text-slate-400'} />
          <span className={`text-[10px] font-black uppercase tracking-widest ${typeColorMap[caseData.type.toLowerCase()] || 'text-slate-500'}`}>
            Área: {caseData.type}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-1">
          {caseData.tags.slice(0, 3).map(tag => (
            <span key={tag} className="px-2 py-0.5 bg-slate-50 dark:bg-slate-800 text-slate-500 text-[10px] rounded border border-slate-100 dark:border-slate-700">
              {tag}
            </span>
          ))}
          {caseData.tags.length > 3 && (
            <span className="text-[10px] text-slate-400">+{caseData.tags.length - 3}</span>
          )}
        </div>
      </div>
      ...
      <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between mt-auto">
        <div className="flex items-center gap-1.5 text-slate-400">
          <Calendar size={12} />
          <span className="text-[10px] font-medium uppercase">{formatDate(caseData.started_at)}</span>
        </div>
