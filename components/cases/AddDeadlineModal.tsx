...
export const AddDeadlineModal: React.FC<AddDeadlineModalProps> = ({ caseId, processNumber, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (formData.deadline_date) {
      const diff = Math.ceil((new Date(formData.deadline_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const existing = JSON.parse(localStorage.getItem('legaltech_deadlines') || '[]');
    localStorage.setItem('legaltech_deadlines', JSON.stringify([...existing, newDeadline]));
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-300">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Processo Relacionado</label>
            <div className="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 text-slate-500 cursor-not-allowed">
              <Lock size={14} />
              <span className="text-sm font-mono font-bold">{processNumber}</span>
            </div>
          </div>

          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Título do Prazo*</label>
            <input 
              required
              type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Data Limite*</label>
              <input 
                required
                type="date"
                className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl focus:ring-2 focus:ring-primary-500 dark:text-white text-sm"
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Prioridade*</label>
              <select 
                required
                className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl focus:ring-2 focus:ring-primary-500 dark:text-white text-sm"
          ...
          {daysRemaining !== null && (
            <div className={`p-3 rounded-xl flex items-center gap-3 border ${
              daysRemaining < 0 ? 'bg-red-50 border-red-100 text-red-600 dark:bg-red-900/10 dark:border-red-900/30' : 
              daysRemaining <= 3 ? 'bg-orange-50 border-orange-100 text-orange-600 dark:bg-orange-900/10 dark:border-orange-900/30' : 
              ...
              <span className="text-xs font-bold uppercase tracking-widest">
                {daysRemaining < 0 ? `Vencido há ${Math.abs(daysRemaining)} dias` : 
                 daysRemaining === 0 ? 'Vence HOJE' : `Faltam ${daysRemaining} dias`}
              </span>
            </div>
          )}

          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Descrição / Observações</label>
            <textarea 
              rows={3}
