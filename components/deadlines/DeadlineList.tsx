...
import { getDeadlineStatus, getPriorityColor } from '../../utils/deadlineCalculations';
...
export const DeadlineList: React.FC<DeadlineListProps> = ({ 
  deadlines, onCheckComplete, onRowClick, onEdit, onDelete 
}) => {
  ...
  if (deadlines.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in">
        ...
        <h3 className="text-xl font-bold dark:text-white">Nenhum prazo encontrado</h3>
        <p className="text-slate-500 mt-2 max-w-xs">Ajuste os filtros ou crie um novo prazo para acompanhar seu andamento.</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
            <tr>
              ...
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Processo / Cliente</th>
                ...
                >
                    ...
                    {isPendente ? (
                      ...
                      >
                        {completingId === d.id ? (
                          ...
                          <Check size={12} className="text-slate-200 group-hover:text-green-500 transition-colors" />
                        )}
                      </button>
                    ) : (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-[10px] font-black uppercase tracking-tighter`} style={{ color }}>{text}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm font-bold dark:text-white group-hover:text-primary-600 transition-colors">{d.title}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-mono font-bold text-slate-500 dark:text-slate-400">{d.case?.process_number || 'S/ Processo'}</span>
                      <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">{d.case?.client.name || 'S/ Cliente'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold dark:text-slate-200">{format(new Date(d.deadline_date), 'dd/MM/yyyy')}</span>
                  ...
                  <td className="px-6 py-4">
                    <span 
                      ...
                      style={{ color: getPriorityColor(d.priority), borderColor: `${getPriorityColor(d.priority)}20`, backgroundColor: `${getPriorityColor(d.priority)}10` }}
                    >
                      {d.priority}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => onEdit(d)} className="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-all" title="Editar">
                        <Edit size={16} />
                      </button>
                      <button onClick={() => onDelete(d.id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Deletar">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
