...
export const CreateDeadlineModal: React.FC<CreateDeadlineModalProps> = ({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({
        ...
        deadline_date: new Date(Date.now() + 5 * 86400000).toISOString().split('T')[0],
        priority: 'média',
        description: ''
      });
    }
  ...
  const priorityOptions = [
    ...
    { id: 'urgente', label: 'Urgente', icon: AlertTriangle, color: 'text-red-500' },
  ];
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <form id="create-deadline-form" onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Prioridade</label>
