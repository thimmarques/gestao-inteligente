...
import { Search, X, Filter, ChevronDown, Calendar, Briefcase, User } from 'lucide-react';
...
export const DeadlineFiltersBar: React.FC<DeadlineFiltersBarProps> = ({ onFilterChange, resultsCount }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [openDropdown, setOpenDropdown] = useState<'priority' | 'process' | null>(null);
  ...
  const isFiltered = searchTerm || selectedPriorities.length > 0 || selectedProcess !== 'todos';
    ...
    <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 bg-white dark:bg-slate-900 p-4 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative">
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-3 rounded-2xl border text-[10px] font-black uppercase tracking-widest transition-all ${
              selectedPriorities.length > 0 ? 'bg-primary-50 border-primary-200 text-primary-700' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-400'
            }`}
          >
            Prioridade
            {selectedPriorities.length > 0 && <span className="bg-primary-600 text-white w-4 h-4 rounded-full flex items-center justify-center text-[8px]">{selectedPriorities.length}</span>}
          ...
          {openDropdown === 'priority' && (
            <div className="absolute top-full left-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 p-2 animate-in zoom-in-95">
              {priorities.map(p => (
                ...
                >
                  <div className={`w-3 h-3 rounded-full ${selectedPriorities.includes(p) ? 'bg-primary-600 ring-4 ring-primary-500/20' : 'bg-slate-200 dark:bg-slate-700'}`} />
                  {p}
                </button>
              ))}
            </div>
        ...
        <div className="px-4 border-l border-slate-200 dark:border-slate-800">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{resultsCount} Resultados</span>
        </div>
      </div>
