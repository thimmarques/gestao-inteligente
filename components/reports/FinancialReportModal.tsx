...
import { 
  X, Calendar, Check, DollarSign, FileText, 
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col animate-in zoom-in-95 duration-300 max-h-[90vh]">
        ...
        <div className="px-10 pt-10 pb-6 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-black dark:text-white tracking-tight">Relatório Financeiro</h2>
                <p className="text-sm font-medium text-slate-500">Configuração de exportação</p>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
          <section className="space-y-6">
            ...
            <PeriodPresets onSelect={(start, end) => setConfig({ ...config, startDate: start, endDate: end })} />
          </section>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <button type="button" onClick={() => setConfig({ ...config, format: 'pdf' })} className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${config.format === 'pdf' ? 'border-primary-600 bg-primary-50/50 dark:bg-primary-900/20' : 'border-slate-100 dark:border-slate-800'}`}>
              ...
              <button type="button" onClick={() => setConfig({ ...config, format: 'excel' })} className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${config.format === 'excel' ? 'border-green-600 bg-green-50/50 dark:bg-green-900/20' : 'border-slate-100 dark:border-slate-800'}`}>
        ...
        <div className="px-10 py-8 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-end">
          <button onClick={() => onGenerate(config)} className="flex items-center gap-3 px-12 py-4 bg-primary-600 hover:bg-primary-700 text-white rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-primary-500/20 transition-all active:scale-95">
            <Download size={24} />
            Gerar Relatório
          </button>
        </div>
