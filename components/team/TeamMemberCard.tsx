...
const roleConfig: Record<string, { icon: any, color: string, bg: string, label: string }> = {
  admin: { icon: Crown, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-900/30', label: 'Admin' },
  advogado: { icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/30', label: 'Advogado' },
  lawyer: { icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/30', label: 'Advogado' },
  assistente: { icon: UserCog, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/30', label: 'Assistente' },
  assistant: { icon: UserCog, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/30', label: 'Assistente' },
};

export const TeamMemberCard: React.FC<TeamMemberCardProps> = ({ member, onClick, currentUserId }) => {
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center">
          <span className="text-primary-600 font-black text-base tabular-nums">{member.stats?.active_cases || 0}</span>
          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest text-center mt-1">Ativos</span>
        </div>
        <div className="p-3 bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center">
          <span className="text-green-600 font-black text-base tabular-nums">{member.stats?.success_rate || 0}%</span>
          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest text-center mt-1">Êxito</span>
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
        <div className="flex items-center justify-between">
          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">LegalTech Equipe</span>
