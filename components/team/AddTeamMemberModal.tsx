...
export const AddTeamMemberModal: React.FC<AddTeamMemberModalProps> = ({
  ...
  const roles = [
    ...
    },
    {
      ...
      description:
        "Auxilia advogados em rotinas, sem acesso a dados sensíveis.",
  ...
  const isStep2Valid =
    ...
    (role !== "advogado" || formData.oab.length >= 5);
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className="flex gap-1">
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className={`h-1 w-8 rounded-full transition-all ${step >= i ? "bg-primary-600" : "bg-slate-100 dark:bg-slate-800"}`}
                    />
                  ))}
                </div>
          ...
          {step === 1 && (
            <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
              <h3 className="text-lg font-bold dark:text-white">
                Qual será a função deste membro?
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {roles.map((r) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center mb-6 shadow-inner ${r.color} ${isActive ? "bg-white dark:bg-slate-800" : "bg-slate-50 dark:bg-slate-800"}`}
                      >
                        <Icon size={32} />
                      </div>
                      <h4 className="text-base font-black dark:text-white mb-2">
                        {r.label}
                      </h4>
                      ...
                      <div className="space-y-1 w-full border-t border-slate-200 dark:border-slate-700 pt-4">
                        {r.perms.map((p, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-1.5 text-[9px] font-bold text-slate-400 uppercase tracking-tighter"
                          >
                      ...
                      {isActive && (
                        <div className="absolute -top-2 -right-2 w-8 h-8 bg-primary-600 text-white rounded-full flex items-center justify-center shadow-lg animate-in zoom-in">
          ...
          {step === 2 && (
            <div className="space-y-10 animate-in slide-in-from-right-4 duration-300 pb-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">
                    Inscrição OAB {role !== "assistente" && "*"}
                  </label>
                  <input
                    type="text"
                    required={role !== "assistente"}
                    value={formData.oab}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        oab: e.target.value.toUpperCase(),
                      })
                    }
          ...
          {step === 3 && (
            <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <PermissionToggle
                      ...
                      sub="Permite cadastrar processos e casos"
                      checked={permissions.can_create_cases}
                      onChange={() => togglePermission("can_create_cases")}
                      disabled={role === "assistente"}
                    />
                    <PermissionToggle
                      label="Editar todos os processos"
                      sub="Permite alterar dados de qualquer caso"
                      checked={permissions.can_edit_cases}
                      onChange={() => togglePermission("can_edit_cases")}
                      disabled={role !== "admin"}
                    />
                    <PermissionToggle
                      label="Deletar processos"
                      sub="Ação crítica de exclusão definitiva"
                      checked={permissions.can_delete_cases}
                      onChange={() => togglePermission("can_delete_cases")}
                      disabled={role !== "admin"}
                    />
                    <PermissionToggle
                      label="Ver processos de todos"
                      sub="Caso OFF, vê apenas os seus processos"
                      checked={permissions.can_view_all_cases}
                      onChange={() => togglePermission("can_view_all_cases")}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div className="space-y-4">
                    ...
                    <PermissionToggle
                      ...
                      sub="Cadastrar e editar base de clientes"
                      checked={permissions.can_create_cases}
                      onChange={() => {}}
                      disabled={role === "assistente"}
                    />
                  ...
                  <div className="space-y-4">
                    ...
                    <PermissionToggle
                      ...
                      sub="Lançamentos de receitas e despesas"
                      checked={permissions.can_manage_finance}
                      onChange={() => togglePermission("can_manage_finance")}
                      disabled={role === "assistente"}
                    />
                    <PermissionToggle
                      label="Gerenciar Equipe"
                      sub="Adicionar ou remover membros"
                      checked={permissions.can_manage_team}
                      onChange={() => togglePermission("can_manage_team")}
                      disabled={role !== "admin"}
                    />
                  </div>
                </div>
              ...
              <div className="p-6 border-2 border-slate-100 dark:border-slate-800 rounded-[2rem] bg-white dark:bg-slate-900 shadow-inner">
                <h4 className="text-xs font-black dark:text-white uppercase tracking-widest mb-4">
                  Resumo do Perfil
                </h4>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(permissions)
                    ...
                    .map(([k]) => (
                      <div
                        key={k}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 dark:bg-green-900/20 text-green-600 rounded-xl text-[9px] font-black uppercase border border-green-100 dark:border-green-800"
                      >
...
const PermissionToggle = ({
  ...
  <div
    className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${disabled ? "opacity-40 grayscale pointer-events-none" : "hover:border-primary-200 dark:hover:border-primary-900/30"}`}
  >
    <div className="space-y-0.5">
      <p className="text-xs font-bold dark:text-white uppercase tracking-tight">
        {label}
      </p>
    ...
    >
      <div
        className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-all shadow-md ${checked ? "translate-x-6" : ""}`}
      />
    </button>
  </div>
);
