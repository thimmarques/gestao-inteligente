...
export const TeamProductivityChart: React.FC<TeamProductivityChartProps> = ({ members }) => {
    ...
    <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-8">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-black dark:text-white uppercase tracking-widest">Produtividade Comparativa</h3>
        </div>
           ...
           {['Este Mês', 'Últimos 3 Meses', 'Este Ano'].map(p => (
             <button
              ...
              className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                period === p.toLowerCase().replace(/ /g, '-') ? 'bg-white dark:bg-slate-700 text-primary-600 shadow-sm' : 'text-slate-400'
              }`}
             >
              {p}
             </button>
      ...
      <div className="h-[350px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
            ...
            />
            <Tooltip 
              contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '1.5rem', color: '#fff', fontSize: '12px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}
            ...
            />
            <Bar dataKey="processos" name="Processos Ativos" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={32} animationDuration={1500} />
            <Bar dataKey="concluidos" name="Concluídos" fill="#22c55e" radius={[6, 6, 0, 0]} barSize={32} animationDuration={2000} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="pt-6 border-t border-slate-100 dark:border-slate-800 flex justify-center gap-8">
         <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Em andamento</span>
         </div>
         <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-green-500" />
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Eficiência Final</span>
         </div>
      </div>
    </div>
  );
