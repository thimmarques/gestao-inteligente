...
import { 
  X, Calendar, Clock, MapPin, Video, Edit, Trash2, 
  CheckCircle2, AlertCircle, ExternalLink, Shield, 
  User, Briefcase, FileText, Globe, Loader2
} from 'lucide-react';
...
interface EventDetailsModalProps {
  ...
  onStatusUpdate: (id: string, status: ScheduleEvent['status']) => Promise<void>;
}

export const EventDetailsModal: React.FC<EventDetailsModalProps> = ({ 
  event, isOpen, onClose, onEdit, onDelete, onStatusUpdate 
}) => {
  ...
  const handleDelete = async () => {
    if (confirm("Tem certeza que deseja deletar este evento permanentemente? Esta ação não pode ser desfeita.")) {
      setIsDeleting(true);
      try {
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="px-8 pt-8 pb-4 flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest opacity-60 dark:text-white" style={{ color: color }}>{event.type}</span>
              <h2 className="text-xl font-bold dark:text-white leading-tight">{event.title}</h2>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => onEdit(event)} className="p-2.5 bg-slate-50 dark:bg-slate-800 hover:bg-primary-50 dark:hover:bg-primary-900/20 text-slate-400 hover:text-primary-600 rounded-xl transition-all">
              <Edit size={20} />
            </button>
            <button onClick={handleDelete} disabled={isDeleting} className="p-2.5 bg-slate-50 dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-500 rounded-xl transition-all">
              {isDeleting ? <Loader2 size={20} className="animate-spin" /> : <Trash2 size={20} />}
            </button>
            <button onClick={onClose} className="p-2.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-all">
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="px-8 py-6 space-y-6 overflow-y-auto max-h-[60vh] custom-scrollbar">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50/50 dark:bg-slate-800/20 rounded-3xl border border-slate-100 dark:border-slate-800">
             <div className="flex items-center gap-3">
               <div className={`w-3 h-3 rounded-full animate-pulse ${event.status === 'agendado' ? 'bg-blue-500' : event.status === 'concluído' ? 'bg-green-500' : 'bg-slate-400'}`} />
               <span className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-300">Status: {event.status}</span>
             </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-slate-500 flex items-center gap-1">
                <Clock size={12} /> {startDate.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })} - {endDate.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })} ({durationText})
              </p>
            </div>
          </div>
          ...
          {(event.location || event.virtual_link) && (
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400">
                {event.virtual_link ? <Video size={20} /> : <MapPin size={20} />}
              </div>
              <div className="flex-1 min-w-0">
                {event.virtual_link ? (
                  <a href={event.virtual_link} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-primary-600 hover:underline flex items-center gap-1.5 truncate">
                    Entrar na Reunião <ExternalLink size={14} />
                  </a>
                ) : (
                  <p className="text-sm font-bold dark:text-white truncate">{event.location}</p>
                )}
                <p className="text-xs font-medium text-slate-500">{event.virtual_link ? 'Link Virtual' : 'Endereço Presencial'}</p>
              </div>
            </div>
          )}
          ...
          {event.case_id && (
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold dark:text-white">Processo {event.case_id}</p>
                <p className="text-xs font-medium text-slate-500">Cliente ID {event.client_id || 'N/A'}</p>
              </div>
            </div>
          )}
          ...
          {event.description && (
            <div className="space-y-2 pt-4 border-t border-slate-100 dark:border-slate-800">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Descrição</h4>
        ...
        <div className="px-8 py-8 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-3">
          {event.status === 'agendado' ? (
            <>
              ...
              >
                {isUpdatingStatus ? <Loader2 size={20} className="animate-spin" /> : <CheckCircle2 size={20} />}
