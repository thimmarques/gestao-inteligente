...
import { Filter, X, Check, Users, Clock, Gavel, Calendar as CalendarIcon, ShieldCheck } from 'lucide-react';
...
export const ScheduleFiltersBar: React.FC<ScheduleFiltersBarProps> = ({ filters, onFilterChange, isOpen }) => {
  ...
  const hasFilters = (filters.types?.length || 0) > 0 || (filters.status?.length || 0) > 0 || filters.showOnlyMine;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm animate-in slide-in-from-top-4 duration-300 space-y-6">
      <div className="flex items-center justify-between">
        ...
        {hasFilters && (
          <button 
            onClick={() => onFilterChange({ types: [], status: [], showOnlyMine: false })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-3">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tipos de Evento</p>
        ...
        {/* Status */}
        <div className="space-y-3">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Situação</p>
          <div className="flex flex-wrap gap-2">
            {statusOptions.map(s => {
        ...
        {/* Filtros Pessoais */}
        <div className="space-y-3">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Preferências</p>
          <button
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg ${filters.showOnlyMine ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-400'}`}>
                <ShieldCheck size={16} />
              </div>
              <span className="text-xs font-bold uppercase tracking-tight">Ver apenas meus eventos</span>
            </div>
            <div className={`w-10 h-5 rounded-full relative transition-all ${filters.showOnlyMine ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-600'}`}>
              <div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-all ${filters.showOnlyMine ? 'translate-x-5' : ''}`} />
            </div>
          </button>
        </div>
      </div>
