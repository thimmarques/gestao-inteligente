...
import { Clock, MapPin, Video, Eye, ChevronRight, Calendar as CalendarIcon } from 'lucide-react';
...
export const AgendaListView: React.FC<AgendaListViewProps> = ({ events, onEventClick }) => {
  ...
  if (dates.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in">
        ...
        <h3 className="text-xl font-bold dark:text-white">Nenhum evento agendado</h3>
        <p className="text-slate-500 mt-2">Você não possui compromissos nos próximos dias.</p>
      </div>
    );
  }
          ...
          <div key={dateKey} className="space-y-4">
            <header className="flex items-center gap-4 px-2">
              <div className="flex flex-col">
                <h3 className={`text-lg font-black dark:text-white ${isToday ? 'text-primary-600' : ''}`}>
                  {dateObj.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}
                </h3>
                {isToday && <span className="text-[10px] font-black uppercase text-primary-500 tracking-[0.2em]">Hoje</span>}
              </div>
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-1 items-center gap-6">
                      <div className="w-16 flex flex-col items-center justify-center border-r border-slate-100 dark:border-slate-800 pr-6 shrink-0">
                        <span className="text-xl font-black dark:text-white tabular-nums">{startTime}</span>
                        <span className="text-[10px] font-bold text-slate-400 tabular-nums">{endTime}</span>
                      </div>
                      
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md" style={{ backgroundColor: `${color}15`, color: color }}>
                            {event.type}
                          </span>
                          {event.status === 'concluído' && (
                            <span className="text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 bg-green-100 text-green-700 rounded-md">Concluído</span>
                          )}
                        </div>
                        <h4 className="text-base font-bold dark:text-white truncate">{event.title}</h4>
                        <div className="flex flex-wrap gap-x-4 gap-y-1">
                          ...
                          <span className="flex items-center gap-1.5 text-xs text-slate-400 font-medium truncate">
                            <CalendarIcon size={14} /> Proc. {event.case_id || 'N/A'}
                          </span>
                        </div>
                      </div>
