...
export const ImportGoogleModal: React.FC<ImportGoogleModalProps> = ({ isOpen, onClose, onImportComplete }) => {
  ...
  const handleSearch = async () => {
    setLoading(true);
    const events = await googleCalendarService.listEvents(new Date(dateRange.start), new Date(dateRange.end));
  ...
  const handleImport = async () => {
    setImporting(true);
    const eventsToImport = availableEvents.filter(e => selectedIds.includes(e.id));
        ...
        <div className="px-10 pt-10 pb-6 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-black dark:text-white tracking-tight">Importar do Google</h2>
                <p className="text-sm font-medium text-slate-500">Sincronize sua agenda externa com o LegalTech.</p>
              </div>
            </div>
            ...
            <div className="space-y-10 animate-in slide-in-from-right-4 duration-300">
              <div className="space-y-6">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Calendar size={14} className="text-primary-500" /> Período de Busca
                </h3>
              ...
              <div className="p-6 bg-blue-50 dark:bg-blue-900/10 rounded-3xl border border-blue-100 dark:border-blue-800 flex gap-4">
                ...
                <p className="text-sm text-primary-900 dark:text-primary-300 font-medium leading-relaxed">
                  O LegalTech buscará todos os eventos públicos e privados da sua conta Google principal no período selecionado.
                </p>
              </div>
                ...
                <label className="flex items-center gap-2 cursor-pointer group">
                  <div 
                    ...
                    className={`w-5 h-5 rounded flex items-center justify-center border-2 transition-all ${
                      selectedIds.length === availableEvents.length ? 'bg-primary-600 border-primary-600' : 'border-slate-200 dark:border-slate-700 group-hover:border-primary-400'
                    }`}
                  >
                    {selectedIds.length === availableEvents.length && <Check size={12} className="text-white" strokeWidth={4} />}
                  </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`w-5 h-5 rounded flex items-center justify-center border-2 transition-all ${
                        isSelected ? 'bg-primary-600 border-primary-600 shadow-sm' : 'border-slate-200 dark:border-slate-700'
                      }`}>
                        {isSelected && <Check size={10} className="text-white" strokeWidth={4} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold dark:text-white truncate">{event.summary}</p>
