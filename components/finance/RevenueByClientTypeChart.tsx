...
export const RevenueByClientTypeChart: React.FC<
  RevenueByClientTypeChartProps
> = ({ revenues }) => {
  const data = useMemo(() => {
    const particular = revenues
      .filter(
        (r) => (r.client as any)?.type === 'particular' && r.status === 'pago'
      )
      .reduce((sum, r) => sum + r.amount, 0);

    const defensoria = revenues
      .filter(
        (r) => (r.client as any)?.type === 'defensoria' && r.status === 'pago'
      )
      .reduce((sum, r) => sum + r.amount, 0);
