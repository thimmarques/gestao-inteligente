...
export const ForecastAlerts: React.FC<ForecastAlertsProps> = ({ forecastData }) => {
  const alerts = useMemo(() => {
    ...
    const revenueChange = ((lastMonth.projected_revenue - firstMonth.projected_revenue) / firstMonth.projected_revenue) * 100;
    ...
    if (revenueChange < -20) {
      alertList.push({
        ...
        suggestions: ['Planeje ações de marketing jurídico imediatamente', 'Avalie contratos de retainer prestes a vencer'],
        icon: <Info size={24} />
      });
      ...
      alertList.push({
        ...
        suggestions: ['Ajuste a infraestrutura para suportar a demanda', 'Considere reinvestir o excedente em tecnologia'],
      ...
      {alerts.map((alert, i) => (
        <div 
          ...
          className={`p-6 rounded-[2rem] border-2 flex gap-6 items-start shadow-sm ${
            alert.type === 'danger' ? 'bg-red-500/10 border-red-500/20 text-red-700' :
        ...
        >
          ...
          <div className="flex-1 space-y-2">
            <h4 className="text-base font-black uppercase tracking-tight">{alert.title}</h4>
            <p className="text-sm font-medium opacity-80">{alert.message}</p>
            <ul className="space-y-1">
              {alert.suggestions.map((s, idx) => (
                <li key={idx} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest opacity-60">
