...
import { X, Save, Search, Loader2, DollarSign, TrendingUp, TrendingDown, Calendar, FileText } from 'lucide-react';
...
export const CreateFinanceModal: React.FC<CreateFinanceModalProps> = ({ isOpen, onClose, onSuccess }) => {
    const { lawyer } = useApp();
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-300">
                ...
                <div className="px-10 pt-10 pb-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-black dark:text-white tracking-tight">Novo Registro</h2>
                        <p className="text-sm font-medium text-slate-500">Controle suas receitas e despesas.</p>
                    </div>
                ...
                <form id="create-finance-form" onSubmit={handleSubmit} className="p-10 space-y-8 overflow-y-auto max-h-[70vh] custom-scrollbar">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Título do Lançamento</label>
                        <input
                            required
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Valor (R$)</label>
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Data</label>
                            <div className="relative">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Vincular Processo (Opcional)</label>
                        <select
                            className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl focus:ring-2 focus:ring-primary-500 dark:text-white text-sm outline-none shadow-sm transition-all"
                        ...
                        >
                            ...
                            {cases.map((c: any) => (
                                <option key={c.id} value={c.id}>{c.process_number} - {c.client?.name}</option>
                            ))}
                        </select>
                    </div>

                    <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">Descrição / Notas</label>
                        <textarea
                            rows={3}
                ...
                <div className="px-10 py-8 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    <button type="button" onClick={onClose} className="px-6 py-3 text-sm font-bold text-slate-400 hover:text-slate-600 uppercase tracking-widest transition-all">Cancelar</button>
                    <button
