...
import { 
  X, Edit, User, Mail, Phone, MapPin, 
  Briefcase, DollarSign, Files, Copy, Clock,
...
export const ClientDetailsModal: React.FC<ClientDetailsModalProps> = ({ 
  client, isOpen, onClose, onEdit 
}) => {
  const [activeTab, setActiveTab] = useState<'geral' | 'processos' | 'financeiro' | 'documentos'>('geral');
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-4xl h-[90vh] rounded-[3rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col animate-in slide-in-from-bottom-8 duration-500">
          ...
          <div className="flex gap-8">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <h2 className="text-3xl font-bold dark:text-white tracking-tight">{client.name}</h2>
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${
                  client.status === 'ativo' ? 'bg-green-100 text-green-700 dark:bg-green-900/30' : 'bg-slate-100 text-slate-500'
                }`}>
                  {client.status}
                </span>
              </div>
              <div className="flex items-center gap-4">
                 <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase inline-flex items-center gap-1 ${
                  client.type === ClientType.PARTICULAR 
                    ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' 
                    : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                }`}>
                  {client.type === ClientType.PARTICULAR ? <User size={10} /> : <Shield size={10} />}
                  {client.type}
                </span>
        ...
        <div className="px-10 border-b border-slate-100 dark:border-slate-800 flex items-center gap-8 overflow-x-auto whitespace-nowrap scrollbar-hide shrink-0">
          {[
            ...
            { id: 'processos', label: 'Processos', icon: <Briefcase size={16} /> },
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-2 space-y-8">
                  <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-8">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.3em]">Informações de Contato</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      <div className="space-y-1 group">
                        <p className="text-[10px] font-bold text-slate-400 uppercase">CPF/CNPJ</p>
                        <div className="flex items-center gap-2">
                          <p className="font-mono text-sm dark:text-white">{formatCPF(client.cpf_cnpj)}</p>
                          <button onClick={() => copyToClipboard(client.cpf_cnpj)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-300 hover:text-primary-500 transition-colors opacity-0 group-hover:opacity-100">
                            <Copy size={12} />
                          </button>
                        </div>
                      </div>
                      <div className="space-y-1">
                        ...
                        <a href={`mailto:${client.email}`} className="text-sm font-medium text-primary-600 hover:underline flex items-center gap-2">
                          <Mail size={14} />
                      ...
                      <div className="space-y-1">
                        ...
                        <a href={`tel:${client.phone}`} className="text-sm font-bold dark:text-white flex items-center gap-2">
                          <Phone size={14} className="text-primary-500" />
                          {formatPhone(client.phone)}
                        </a>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] font-bold text-slate-400 uppercase">Cidade / Estado</p>
                        <p className="text-sm font-medium dark:text-slate-300">São Paulo, SP</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.3em]">Notas e Observações</h3>
                    <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed italic">
                      "{client.notes || 'Nenhuma nota adicional para este cliente.'}"
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div className="bg-primary-600 p-8 rounded-[2.5rem] text-white shadow-xl shadow-primary-500/20">
                    <h4 className="text-[10px] font-bold uppercase tracking-widest opacity-60 mb-6">Perfil Estratégico</h4>
                    {client.type === ClientType.PARTICULAR ? (
                      <div className="space-y-6">
                        <div className="flex items-center gap-4">
                          <div className="p-2 bg-white/20 rounded-xl"><DollarSign size={20} /></div>
                          <div>
                            <p className="text-[10px] uppercase font-bold opacity-60">Ticket Médio</p>
                            <p className="text-lg font-bold">{formatCurrency(client.financial_profile?.hourly_rate || 0)}/h</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-4">
                          <div className="p-2 bg-white/20 rounded-xl"><Clock size={20} /></div>
                          <div>
                            <p className="text-[10px] uppercase font-bold opacity-60">Retainer</p>
                            <p className="text-lg font-bold">{formatCurrency(client.financial_profile?.retainer_fee || 0)}/mês</p>
                          </div>
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-6">
                         <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-[10px] uppercase font-bold opacity-60">Comarca</p>
                            <p className="text-lg font-bold">{client.financial_profile?.comarca || 'N/A'}</p>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center gap-4">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold dark:text-white">{client.process_count}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Processos Ativos</p>
                    </div>
            ...
            {activeTab !== 'geral' && (
              <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800 text-center">
                <div className="w-20 h-20 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-300 dark:text-slate-700 mb-6">
                  {activeTab === 'processos' ? <Briefcase size={40} /> : activeTab === 'financeiro' ? <DollarSign size={40} /> : <Files size={40} />}
                </div>
                <h4 className="text-xl font-bold dark:text-white mb-2">Dados de {activeTab}</h4>
                <p className="text-slate-500 max-w-xs text-sm">Os registros detalhados de {activeTab} para este cliente estão sendo reconciliados.</p>
              </div>
            )}
          </div>
        </div>
