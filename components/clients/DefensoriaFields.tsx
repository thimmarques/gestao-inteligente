...
export const DefensoriaFields: React.FC<DefensoriaFieldsProps> = ({ data, onChange }) => {
  return (
      ...
      <div className="bg-slate-50/50 dark:bg-slate-800/30 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Nº Processo Nomeação</label>
            <input
              type="text"
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Comarca / Foro</label>
            <input
              type="text"
              value={data.comarca || ''}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Data de Nomeação</label>
            <input
              type="date"
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Tipo de Processo</label>
            <select
      ...
      <div className="bg-slate-50/50 dark:bg-slate-800/30 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="flex items-center justify-between p-3 bg-white dark:bg-slate-900 rounded-xl">
            <span className="text-sm font-medium dark:text-slate-300">Declaração Hipossuficiência</span>
            <button
              type="button"
              onClick={() => onChange('has_hipossuficiencia', !data.has_hipossuficiencia)}
            ...
            >
              <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${data.has_hipossuficiencia ? 'translate-x-6' : ''}`} />
            </button>
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Situação Habitacional</label>
            <input
              type="text"
          ...
          <div className="col-span-full space-y-1.5">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Observações Sociais</label>
            <textarea
              value={data.social_notes || ''}
              onChange={(e) => onChange('social_notes', e.target.value.slice(0, 500))}
              rows={4}
