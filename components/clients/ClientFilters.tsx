...
export const ClientFiltersBar: React.FC<ClientFiltersProps> = ({ onFilterChange, onExport }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [openDropdown, setOpenDropdown] = useState<'status' | 'order' | null>(null);
  ...
  const updateOrder = (sortBy: ClientFilters['sortBy'], direction: ClientFilters['sortDirection']) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 py-4">
      ...
      <div className="relative">
        ...
        >
          <Filter size={16} />
          Status {filters.status !== 'todos' && <span className="ml-1 w-2 h-2 bg-primary-600 rounded-full" />}
        </button>
        {openDropdown === 'status' && (
      ...
      <div className="relative">
        ...
        {openDropdown === 'order' && (
          <div className="absolute top-full right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-20 p-2 animate-in fade-in zoom-in-95">
            <button onClick={() => updateOrder('name', 'asc')} className="w-full text-left px-4 py-2 rounded-lg text-sm hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300">Nome (A-Z)</button>
            <button onClick={() => updateOrder('name', 'desc')} className="w-full text-left px-4 py-2 rounded-lg text-sm hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300">Nome (Z-A)</button>
            <button onClick={() => updateOrder('created_at', 'desc')} className="w-full text-left px-4 py-2 rounded-lg text-sm hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300">Mais Recentes</button>
            <button onClick={() => updateOrder('created_at', 'asc')} className="w-full text-left px-4 py-2 rounded-lg text-sm hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300">Mais Antigos</button>
          </div>
        )}
      </div>
